...
class Fboss():
    ...
    def _fpga_io_operation(self, reg:hex, val=None):
        ...
        try:
            with open(bdf_fd, mode="r+", encoding="utf-8") as fpgaio:
                with contextlib.closing(mmap.mmap(fpgaio.fileno(),              \
                                        length=0, flags=mmap.MAP_SHARED,        \
                                        access=mmap.ACCESS_DEFAULT)) as mmap_obj:
                    if val is not None:
                        mmap_obj.seek(reg)
                        ...
                        mmap_obj.seek(reg)
                        strout = struct.pack('<l', *struct.unpack('>l', mmap_obj.read(4)))
    ...
    def _execute_i2cget(self,
                        bus:int,
                        addr:int,
                        reg:int) -> tuple[bool,list[int]]:
    ...
    def iob_reg_raw_data_show(self):
        ...
        for n in IOB_REGS.keys():
            ...
            print(f'{"":>5}{n:<20s}{"":>6}{hex(reg):<7}{"":>14}'
                  f'0x{regval:<10} \n', end='')
    ...
    def iob_xadc_test(self):
        ...
        for xadc in IOB_XADC.keys():
            ...
            print(f'{"":>3}{xadc:9s}{reg}{"":>6}{"{:.2f}".format(temp):<5}{"":>7}'
    ...
    def iob_up_time_test(self, sleep_time:int=None):
        ...
        if sleep_time:
            ...
            print(f'{"":3}{int(start, 16)}{"":>15}{sleep_time:<5}{"":>7}'
                  f'{int(end, 16):<5}{"":>12}{int(end, 16) - int(start, 16):<5}{"":>5}'
                  f'{stat:<5} \n', end='')
            return start, end
        print(f"IOB FPGA Up time register value: {start} up time: {int(start, 16)}s(encod)\n")
        return start
    ...
    def firmware_version_info(self) -> str:
        ...
        if self._platform == "montblanc":
            ...
            _major, _minor, _patch =                                            \
                    self._cplds_version(I2C_BUS_SCMCPLD, I2C_ADDR_SCMCPLD)
            scm_version = f'{_major[0] & 0x7f}.{_minor[0]}.{_patch[0]}'

            # SMB CPLD version
            _major, _minor, _patch =                                            \
                    self._cplds_version(I2C_BUS_SMBCPLD, I2C_ADDR_SMBCPLD)
            smb_version = f'{_major[0] & 0x7f}.{_minor[0]}.{_patch[0]}'

            # MCB CPLD version
            _major, _minor, _patch =                                            \
                    self._cplds_version(I2C_BUS_MCBCPLD, I2C_ADDR_MCBCPLD)
...
'''
        if self._platform == "janga" or self._platform == "tahan":
            # PWR CPLD Version
            _major, _minor, _patch =                                            \
                    self._cplds_version(I2C_BUS_PWRCPLD, I2C_ADDR_PWRCPLD)
            pwr_version = f'{_major[0] & 0x7f}.{_minor[0]}.{_patch[0]}'

            # SMB CPLD1 Version
            _major, _minor, _patch =                                            \
                    self._cplds_version(I2C_BUS_SMBCPLD1, I2C_ADDR_SMBCPLD1)
            smb1_version = f'{_major[0] & 0x7f}.{_minor[0]}.{_patch[0]}'

            # SMB CPLD2 Version
            _major, _minor, _patch =                                            \
                    self._cplds_version(I2C_BUS_SMBCPLD2, I2C_ADDR_SMBCPLD2)
...
'''
    ...
    def detect_iob_i2c_buses(self):
        ...
        for bus_name in dev_map.keys():
            status, sta_info = i2cbus.scan_verify_i2c_bus(self._platform, "IOB", bus_name, dev_map)
    ...
    def detect_doms_i2c_buses(self):
        ...
        for n in range(max_bus):
            ...
            status, sta_info = i2cbus.scan_verify_i2c_bus(self._platform, "DOM", bus_name, dev_map)
