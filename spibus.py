...
def generate_spidev():
    ...
    for busid in range(8):
        ...
        if not pathlib.Path(drv_path).exists() or                   \
                    not pathlib.Path(bind_path).exists():
            break
...
class SPIBUS():
    ...
    def spi_master_detect(self) -> bool:
        ...
        for busid in range(8):
            ...
            if pathlib.Path(dev_path).exists()                          \
                and pathlib.Path(master_path).exists():
                ...
                if not sta:
                    ...
                    if int(spidevid) != int(masterid):
                        errcode = False
                        status = FMTOUT.format(f"FAIL - UDEV map Error.[spi.{spidevid}]")
            else:
                spibus = spidev_info = spidev_udev = "NA"
                errcode = False
                status =  FMTOUT.format("FAIL - SPI Master or Driver Error.")

            print(f'{busid:>7d}  {spibus:>12}{"":5}{spidev_info:>10} '
    ...
    def spi_scan(self, dev:str) -> bool:
        ...
        while True:
            ...
            if dev == 'iob':
                cmd = f'flashrom -p linux_spi:dev={spidev} -c'               \
            ...
            if not stat:
                ...
                try:
                    vendor, name = SPI_VENDOR_PATTERN.findall(              \
                                    stdout.splitlines()[-1])[0]
                except ValueError:
                    res = False
                    break
                cmd = f'flashrom -p linux_spi:dev={spidev} -c'               \
        ...
        print(f'{dev.upper():>7s} Flash   {self.spi_dict[dev]["bus"]:>3d}   '
              + f'{"".join(map(str, mux)) if mux else "NA":>6}{"":7}{vendor.ljust(7)}  '
              + f' {name.ljust(10)}{size:>9} ', end='')
